...
const AddProduct = () => {
  ...
  const validationSchema = Yup.object({
    ...
    mainImage: Yup.mixed().required("Ảnh chính là bắt buộc"),
    quantity: Yup.number().required("Số lượng sản phẩm là bắt buộc").min(1, "Số lượng phải lớn hơn 0"),
    ...
    shipping: Yup.object({
      freeShippingThreshold: Yup.string().notRequired(),
    }).notRequired(),
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      console.error("Có lỗi khi thêm sản phẩm:", error.response?.data || error.message);
        ...
        >
          {({ values, setFieldValue }) => (
            <Form>
              ...
              <div className="mb-3">
                <label className="form-label">Vận chuyển - Miễn phí vận chuyển từ</label>
                <ReactQuill
                  theme="snow"
                  value={values.shipping.freeShippingThreshold}
                  onChange={(content) => setFieldValue("shipping.freeShippingThreshold", content)}
                  placeholder="Nhập số tiền miễn phí vận chuyển"
                />
              </div>
              ...
              <div className="mb-3">
                ...
                <ReactQuill
                  theme="snow"
                  value={values.promotion.description}
                  onChange={(content) => setFieldValue("promotion.description", content)}
              ...
              <div className="mb-3">
                ...
                {values.mainImage && <p>Ảnh đã chọn: {values.mainImage.name}</p>}
              </div>
